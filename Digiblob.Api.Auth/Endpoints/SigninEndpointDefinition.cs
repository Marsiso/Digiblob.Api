...
public sealed class SigninEndpointDefinition : IEndpointDefinition
{
    public void DefineEndpoints(WebApplication application)
    {
        application.MapPost(ApiRoutes.Posts.Signin, async (
            ...
            .Accepts<SigninRequest>(MediaTypeNames.Application.Json)
            .Produces(StatusCodes.Status201Created, typeof(SigninResponse), MediaTypeNames.Application.Json)
            .Produces(StatusCodes.Status422UnprocessableEntity, typeof(ValidationProblemResponse),
            ...
            .WithSummary("Creates a new user account and performs sign-in")
            .WithDescription(
    ...
    private static async Task SigninHandlerAsync(
        HttpRequest request,
        HttpResponse response,
        CancellationToken cancellationToken,
        [FromServices] ISender sender,
